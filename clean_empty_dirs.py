...
def find_empty_dirs(start_path: str = '.', ignore_dirs: List[str] = None) -> Tuple[List[str], Dict]:
    """
    递归查找所有空目录
    ...
    if not os.path.isdir(start_path):
        print(f"{Colors.RED}错误: 目录 '{start_path}' 不存在或不是一个目录{Colors.RESET}")
        sys.exit(1)
    ...
    try:
        for root, dirs, files in os.walk(start_path, topdown=False):
            ...
            try:
                if root != start_path and os.path.isdir(root) and not os.listdir(root):  # 避免删除起始目录
                    empty_dirs.append(root)
            except (PermissionError, OSError) as e:
                print(f"{Colors.RED}警告: 无法访问目录 '{root}': {e}{Colors.RESET}")
    except Exception as e:
        print(f"{Colors.RED}扫描时发生错误: {e}{Colors.RESET}")
        sys.exit(1)
...
def remove_empty_dirs(dirs: List[str], dry_run: bool = False) -> Tuple[int, List[str], List[str]]:
    """
    删除指定的空目录
    ...
    for dir_path in dirs:
        try:
            ...
            print(f"{Colors.RED}错误{Colors.RESET}: 无法删除 {dir_path}: {e}", file=sys.stderr)
...
def get_user_confirmation(count: int) -> bool:
    ...
    """
    while True:
        response = input(f"\n{Colors.YELLOW}确认删除这 {count} 个空文件夹吗? (y/n): {Colors.RESET}").lower()
...
def print_summary(stats: Dict, count: int, succeeded: List[str], failed: List[str], dry_run: bool):
    """
    打印操作摘要
...
def main():
    parser = argparse.ArgumentParser(description='清除当前目录及其子目录中的所有空文件夹')
    parser.add_argument('-p', '--path', default='.', help='开始搜索的路径，默认为当前目录')
    parser.add_argument('-i', '--ignore', nargs='+', help='要忽略的目录名列表')
    parser.add_argument('-d', '--dry-run', action='store_true', help='仅显示将要删除的目录，不实际删除')
    parser.add_argument('-v', '--verbose', action='store_true', help='显示详细输出')
    parser.add_argument('-y', '--yes', action='store_true', help='不提示确认，直接删除')
    parser.add_argument('-l', '--list', action='store_true', help='删除后列出所有已删除的文件夹')
    parser.add_argument('--no-color', action='store_true', help='禁用彩色输出')
    
    args = parser.parse_args()
    
    # 在Windows上检查是否支持ANSI颜色代码
    if os.name == 'nt':
        try:
            import colorama
            colorama.init()
        except ImportError:
            # 如果没有安装colorama，默认禁用颜色
            args.no_color = True
    ...
    try:
        ...
        if not os.path.exists(start_path):
            print(f"{Colors.RED}错误: 路径 '{args.path}' 不存在{Colors.RESET}")
            sys.exit(1)
        if not os.path.isdir(start_path):
            print(f"{Colors.RED}错误: '{args.path}' 不是一个目录{Colors.RESET}")
            sys.exit(1)
    except Exception as e:
        print(f"{Colors.RED}错误: 无法解析路径 '{args.path}': {e}{Colors.RESET}")
        sys.exit(1)
    
    ignore_dirs = args.ignore or ['.git', '.svn', '.hg', '__pycache__', 'node_modules']
    ...
    try:
        ...
        print(f"\n{Colors.RED}扫描时发生错误: {e}{Colors.RESET}")
        sys.exit(1)
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n{Colors.RED}发生未预期的错误: {e}{Colors.RESET}")
