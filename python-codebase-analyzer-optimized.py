...
class Colors:
    ...
    def disable():
        ...
        for attr in dir(Colors):
            if not attr.startswith('_') and attr != 'disable' and attr != 'supports_color':
                setattr(Colors, attr, "")
...
class AntiPatternDetector:
    ...
    PATTERNS = {
        ...
        "mutable_default": (r"def\s+\w+\s*\(.*=\s*\[\s*\].*\)|\(.*=\s*\{\s*\}.*\)", "使用可变默认参数"),
    ...
    def find_antipatterns(code: str) -> List[Tuple[str, str]]:
        ...
        for name, (pattern, description) in AntiPatternDetector.PATTERNS.items():
            ...
            for match in matches:
                ...
                line = code.splitlines()[line_start-1] if line_start <= len(code.splitlines()) else ""
...
class QualityScoreCalculator:
    ...
    def calculate_file_score(stats: FileStats) -> float:
        ...
        if stats.function_count + stats.method_count > 0:
            complexity_per_function = stats.cyclomatic_complexity / (stats.function_count + stats.method_count)
        ...
        if stats.class_count + stats.function_count + stats.method_count > 0:
            if stats.quality.has_docstring:
                docstring_ratio = stats.docstring_lines / (stats.class_count + stats.function_count + stats.method_count)
        ...
        # 命名规范得分
        if stats.quality.naming_stats.good_names + stats.quality.naming_stats.bad_names > 0:
            naming_ratio = stats.quality.naming_stats.good_names / (stats.quality.naming_stats.good_names + stats.quality.naming_stats.bad_names)
        ...
        # 计算加权总分
        weighted_score = sum(scores[key] * QualityScoreCalculator.WEIGHTS[key] for key in scores)
...
class PythonModuleAnalyzer:
    ...
    def analyze(self) -> FileStats:
        ...
        # 计算质量评分
        self.stats.quality_score = QualityScoreCalculator.calculate_file_score(self.stats)
    ...
    def _analyze_ast(self):
        ...
        # 检查模块文档字符串
        if (len(self.module_ast.body) > 0 and 
            isinstance(self.module_ast.body[0], ast.Expr) and 
        ...
        for node in ast.walk(self.module_ast):
            ...
            # 统计函数和方法
                ...
                for inner_node in ast.walk(node):
                    if isinstance(inner_node, (ast.If, ast.For, ast.While, ast.Try, ast.ExceptHandler)):
                        complexity += 1
                    ...
                    for target in node.targets:
                        if isinstance(target, ast.Name):
                            if NamingConventionChecker.check_variable_name(target.id):
                                ...
                                # 跳过魔术变量和常量
                                if not (target.id.startswith('__') and target.id.endswith('__')) and not target.id.isupper():
                                    naming_stats.bad_names += 1
                                    case_type = NamingConventionChecker.get_name_case_type(target.id)
            ...
            # 检查带类型注解的变量
                ...
                if CONFIG["check_naming"] and isinstance(node.target, ast.Name):
                    if NamingConventionChecker.check_variable_name(node.target.id):
                        ...
                        # 跳过魔术变量和常量
                        if not (node.target.id.startswith('__') and node.target.id.endswith('__')) and not node.target.id.isupper():
                            naming_stats.bad_names += 1
                            naming_stats.naming_issues.append(
                    ...
                    if node.module:  # 跳过无模块的相对导入
                        for name in node.names:
                            if name.name == '*':
                                ...
                                imports.setdefault(node.module, []).append(imported_name)
    ...
    def _detect_antipatterns(self):
        ...
        self.stats.quality.antipatterns = [f"{desc}: {line}" for line, desc in antipatterns]
...
class PythonProjectAnalyzer:
    ...
    def analyze(self) -> Dict:
            ...
            with ThreadPoolExecutor(max_workers=self.config["max_workers"]) as executor:
                ...
                for i, future in enumerate(as_completed(futures)):
                    ...
                    if files_processed % 10 == 0 or files_processed == total_files:
                        ...
                        print(f"\r{Colors.CYAN}已处理 {files_processed} 个文件 "
                              f"({round(elapsed, 1)}秒){Colors.RESET}", end="")
                        sys.stdout.flush()
        ...
        # 添加模块和依赖关系到结果
        result["modules"] = {name: asdict(module) for name, module in self.modules.items()}
        result["dependencies"] = {
    ...
    def _generate_quality_summary(self, result: Dict) -> Dict:
        ...
        for stats in result["stats"].values():
            if isinstance(stats, dict) and stats.get("quality", {}).get("has_docstring", False):
                files_with_docstrings += 1
        
        docstring_coverage = round((files_with_docstrings / total_files) * 100, 1) if total_files > 0 else 0
        
        # 计算类型提示覆盖率
        files_with_type_hints = 0
        for stats in result["stats"].values():
            if isinstance(stats, dict) and stats.get("quality", {}).get("has_type_hints", False):
                files_with_type_hints += 1
        
        type_hint_coverage = round((files_with_type_hints / total_files) * 100, 1) if total_files > 0 else 0
        
        # 获取平均复杂度
        avg_complexity = round(total["complexity"] / (total["functions"] + total["methods"]), 1) \
            if (total["functions"] + total["methods"]) > 0 else 0
            
        # 计算代码注释比例
        comment_ratio = round(total["comment_lines"] / total["code_lines"] * 100, 1) if total["code_lines"] > 0 else 0
    ...
    def _discover_modules(self):
        ...
        for root, dirs, files in os.walk(self.root_path, topdown=True):
            ...
            # 处理目录中的所有Python文件
            for f in files:
                if f.endswith(".py") and not self._is_excluded(f, self.config["exclude_files"]):
                    if f == "__init__.py":
                        continue  # 上面已经处理过了
        ...
        for module_name, module_info in self.modules.items():
            ...
            for imported_module, imported_items in module_info.stats.imports.items():
                ...
                if imported_module in module_name or any(
                    imported_module.startswith(f"{mod_name}.") for mod_name in self.modules
                ):
                    # 找到一个项目本地导入
    ...
    def _analyze_file(self, file_path: Path) -> Tuple[Dict, Dict]:
        ...
        try:
            ...
            # 跳过非Python文件或太大的文件
            if not file_path.suffix == ".py" or file_path.stat().st_size > self.config["max_file_size_mb"] * 1024 * 1024:
...
class DependencyVisualizer:
    ...
    @staticmethod
    def create_dependency_matrix(dependencies: Dict[str, List[str]], modules: List[str]) -> List[List[str]]:
        """创建一个依赖关系矩阵
    ...
    def generate_ascii_dependency_graph(dependencies: Dict[str, List[str]]) -> str:
        ...
        if len(project_modules) <= 10:
            return DependencyVisualizer._generate_simple_graph(dependencies, project_modules)
        
        # 针对大型项目的矩阵依赖图
        return DependencyVisualizer._generate_matrix_graph(dependencies, project_modules)
    
    @staticmethod
    def _generate_simple_graph(dependencies: Dict[str, List[str]], modules: List[str]) -> str:
        """为小型项目生成简单的节点和边图"""
        lines = ["依赖关系图:"]
    ...
    @staticmethod
    def _generate_matrix_graph(dependencies: Dict[str, List[str]], modules: List[str]) -> str:
        """为大型项目生成依赖关系矩阵"""
        matrix, sorted_modules = DependencyVisualizer.create_dependency_matrix(dependencies, modules)
...
class OutputFormatter:
    ...
    def _text_format(result: Dict, config: Dict) -> str:
        """格式化结果为纯文本"""
        output = [f"{Colors.BOLD}{Colors.CYAN}===== Python项目分析报告 ====={Colors.RESET}\n"]
        ...
        if summary:
            ...
            output.append(f"总体质量: {category_color}{score} - {category}{Colors.RESET}")
            output.append(f"文档覆盖率: {summary.get('files_with_docstrings_percent', 0)}%")
            output.append(f"类型提示覆盖率: {summary.get('files_with_type_hints_percent', 0)}%")
        ...
        # 摘要统计
        output.append(f"\n{Colors.BOLD}{Colors.BLUE}=== 项目摘要 ==={Colors.RESET}")
        total = result["total"]
        output.append(f"分析的文件数: {total['files']}")
        output.append(f"总行数: {total['total_lines']}")
        output.append(f"代码行: {total['code_lines']} ({OutputFormatter._percentage(total['code_lines'], total['total_lines'])}%)")
        output.append(f"注释行: {total['comment_lines']} ({OutputFormatter._percentage(total['comment_lines'], total['total_lines'])}%)")
        output.append(f"文档字符串行: {total['docstring_lines']} ({OutputFormatter._percentage(total['docstring_lines'], total['total_lines'])}%)")
        output.append(f"空行: {total['blank_lines']} ({OutputFormatter._percentage(total['blank_lines'], total['total_lines'])}%)")
        ...
        if config["detailed_imports"] and result["dependencies"]:
            ...
            # 生成ASCII依赖图
            dependency_graph = DependencyVisualizer.generate_ascii_dependency_graph(result["dependencies"])
        ...
        if result["stats"]:
            ...
            complex_files = sorted(
                [(path, stats.get("cyclomatic_complexity", 0), stats.get("quality_score", 0)) 
                 for path, stats in result["stats"].items() if isinstance(stats, dict)],
                key=lambda x: x[1],
                reverse=True
            )[:10]
            ...
            for path, complexity, score in complex_files:
                ...
                output.append(f"{path}: 复杂度 {complexity}, 质量评分: {score_color}{score}{Colors.RESET}")
        ...
        if config["detect_antipatterns"]:
            ...
            for path, stats in result["stats"].items():
                if not isinstance(stats, dict):
                    continue
                quality = stats.get("quality", {})
                if isinstance(quality, dict) and quality.get("antipatterns_count", 0) > 0:
                    antipattern_files.append((path, quality["antipatterns_count"], quality.get("antipatterns", [])))
            
            if antipattern_files:
                output.append(f"\n{Colors.BOLD}{Colors.YELLOW}=== 含有反模式的文件 ==={Colors.RESET}")
                for path, count, patterns in sorted(antipattern_files, key=lambda x: x[1], reverse=True)[:10]:
        ...
        if config["check_naming"]:
            ...
            for path, stats in result["stats"].items():
                ...
                if not isinstance(quality, dict):
                    continue
                naming_stats = quality.get("naming_stats", {})
                if isinstance(naming_stats, dict) and naming_stats.get("bad_names", 0) > 0:
                    naming_issue_files.append((path, naming_stats["bad_names"], naming_stats.get("naming_issues", [])))
            
            if naming_issue_files:
                output.append(f"\n{Colors.BOLD}{Colors.YELLOW}=== 命名规范问题 ==={Colors.RESET}")
                for path, count, issues in sorted(naming_issue_files, key=lambda x: x[1], reverse=True)[:10]:
    ...
    def _format_entry(entry: Dict, result: Dict, output: List, level: int):
        ...
        if entry["type"] == "directory":
            ...
            for child in sorted(entry.get("children", []), key=lambda x: (x["type"] != "directory", x["name"])):
                OutputFormatter._format_entry(child, result, output, level + 1)
        else:
            file_info = OutputFormatter._file_info(entry, result)
            if entry["name"].endswith(".py"):
                output.append(f"{prefix}{Colors.CYAN}{entry['name']}{Colors.RESET} {file_info}")
            else:
...
def parse_args():
    ...
    parser.add_argument("path", nargs="?", default=".", help="Python项目路径（默认：当前目录）")
    ...
    parser.add_argument("--max-file-size", type=int, default=10, help="要分析的最大文件大小（MB）")
    parser.add_argument("--no-antipatterns", action="store_false", dest="detect_antipatterns", help="禁用反模式检测")
    parser.add_argument("--no-complexity", action="store_false", dest="calculate_complexity", help="禁用复杂度计算")
    parser.add_argument("--no-docstrings", action="store_false", dest="analyze_docstrings", help="禁用文档字符串分析")
    parser.add_argument("--no-imports", action="store_false", dest="detailed_imports", help="禁用详细导入分析")
    parser.add_argument("--no-naming", action="store_false", dest="check_naming", help="禁用命名规范检查")
    parser.add_argument("--no-color", action="store_false", dest="color_output", help="禁用彩色输出")
